/* tour.js: the state machine that drives one document through the whole park.
 *
 * A "batch" is one complete run: in at the warehouse, round the retrieval ring
 * once per search pass, out at the generation gates, onto a lorry at the audit
 * dock and away to the evaluation centre, then the empty lorry drives back for
 * the next one. The cart's cargo is the document, and it changes at every stop. */
(function (global) {
  'use strict';
  ...
  var CARGO = {
    warehouse: 'raw', parser: 'parsed', cleaner: 'clean', chunker: 'chunks',
    enricher: 'meta', dedup: 'unique', embed: 'vectors', vector: 'indexed',
    filter: 'filtered', query: 'question', rewrite: 'rewritten',
    hybrid: 'candidates', rrf: 'fused', rerank: 'topk', rights: 'rights',
    loopct: 'rights', context: 'prompt', llm: 'answer', cite: 'cited',
    guard: 'safe', dock: 'logged', eval: 'delivered'
    /* nothing for newbatch: the lorry stays visibly empty until startBatch
       hands a fresh cart back to the warehouse */
  };
  ...
  var state = {
    ...
    racks: 0,                  // evaluations delivered, drawn as lit halls in the centre
  ...
  function advanceRoute() {
    if (cart.routeName === 'intake') {
      ...
      /* off the audit dock and onto the road: the cart becomes a lorry here */
